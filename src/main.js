...
document.querySelector('#app').innerHTML = `
  ...
  <div class="feedback-modal" id="feedback-modal" aria-hidden="true"><div class="feedback-card"><button class="close-room" id="close-feedback" aria-label="Close feedback">×</button><p class="eyebrow">SESSION COMPLETE</p><h2>Good work showing up.</h2><p class="feedback-copy" id="feedback-copy">Your practice session is saved locally. Here is your practice snapshot.</p><div class="feedback-stats"><div><strong id="clarity-score">--/5</strong><small>Clarity</small></div><div><strong id="structure-score">--/5</strong><small>Structure</small></div><div><strong id="detail-score">--/5</strong><small>Detail</small></div></div><p class="feedback-tip" id="feedback-tip"></p><button class="primary-button" id="done-feedback">Back to overview</button></div></div>
...
const showFeedback = (feedback) => { document.querySelector('#clarity-score').textContent = `${feedback.clarity}/5`; document.querySelector('#structure-score').textContent = `${feedback.structure}/5`; document.querySelector('#detail-score').textContent = `${feedback.detail}/5`; document.querySelector('#feedback-copy').textContent = feedback.summary; document.querySelector('#feedback-tip').textContent = `Next practice: ${feedback.nextTip}`; feedbackModal.classList.add('open'); feedbackModal.setAttribute('aria-hidden', 'false') }
...
document.querySelector('#screen-toggle').addEventListener('click', () => document.querySelector('#notes-panel').classList.toggle('visible'))
const notes = document.querySelector('#session-notes')
notes.value = localStorage.getItem('roundready-session-notes') || ''
notes.addEventListener('input', () => { localStorage.setItem('roundready-session-notes', notes.value); document.querySelector('#notes-status').textContent = 'Saved just now' })
document.querySelector('#clear-notes').addEventListener('click', () => { notes.value = ''; localStorage.removeItem('roundready-session-notes'); document.querySelector('#notes-status').textContent = 'Autosaves locally' })
